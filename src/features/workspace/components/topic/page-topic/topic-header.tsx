...
import { patchPartialTopicNoteAction } from "@/features/workspace/actions/patch-partial-topic-action";
import { useSubject } from "@/features/workspace/lib/subject-provider";
...
export function TopicHeader({ data }: { data: Topic }) {
  ...
  const [editingField, setEditingField] = useState<keyof typeof values | null>(
    null,
  );
  ...
  const handleSave = (e?: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        ...
        await patchPartialTopicNoteAction(data.id, formData);
  ...
  const cleanInputNumberClass =
    "w-full min-w-0 bg-transparent font-bold text-2xl p-0 border-b-2 border-dashed border-primary/40 focus:border-primary outline-none ring-0 focus:ring-0 transition-colors [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none";
    ...
    <div>
      <div className="group flex items-center min-h-12">
        {editingField === "name" ? (
          <form
            onSubmit={handleSave}
            className="flex items-center gap-3 w-full max-w-2xl animate-in fade-in zoom-in-95 duration-200"
          >
          ...
          <div className="flex items-center gap-2">
            <Link href={`/workspace/subjects/${subject.id}`}>
              <h1
                className="font-semibold text-primary text-xl cursor-pointer rounded-lg px-2 py-1 transition-colors"
                title="Klik untuk mengedit"
              >
                {subject.name}
              </h1>
            </Link>
            {/*<Separator orientation="vertical" className={'rotate-20'}/>*/}
            <span className="w-0.5 bg-border rotate-20 h-10" />
            <h1
              onClick={() => startEditing("name", values.name)}
              className="font-semibold ml-2 text-xl cursor-pointer bg-muted/50 rounded-lg px-2 py-1 transition-colors border border-border border-dashed"
              title="Klik untuk mengedit"
            >
              {values.name}
            </h1>
        ...
        <div className="flex flex-col gap-2 rounded-xl border-2 bg-card p-4 text-card-foreground border-dashed">
          ...
          <div className="min-h-6">
            {editingField === "description" ? (
              <form
                onSubmit={handleSave}
                className="flex flex-col gap-2 animate-in fade-in zoom-in-95 duration-200"
              >
                <textarea
                  ...
                  className="w-full min-h-[80px] bg-background rounded-md border p-2 text-sm outline-none focus:ring-2 focus:ring-ring resize-y text-foreground/80 placeholder:text-muted-foreground"
              ...
              >
                {values.description || (
                  <span className="italic text-muted-foreground">
                    Belum ada deskripsi. Klik untuk menambah...
                  </span>
                )}
              </p>
            )}
            ...
            <div className="flex flex-col justify-between gap-3 rounded-xl border p-4 bg-card">
                ...
                <Tooltip>
                  <TooltipTrigger
                    render={
                      <ArrowDownToLine
                        size={16}
                        className="cursor-help text-foreground/50 hover:text-foreground transition-colors"
                      />
                    }
                  />
                  <TooltipContent>
                    <p>Ambang batas skor terendah</p>
                  </TooltipContent>
                </Tooltip>
              </div>
              <div className="min-h-8 flex items-center">
                {editingField === "minAverageScore" ? (
                  <form
                    onSubmit={handleSave}
                    className="flex items-center gap-2 w-full animate-in fade-in zoom-in-95 duration-200"
                  >
                    <input
                      ref={inputRef as React.RefObject<HTMLInputElement>}
                      type="number"
                    ...
                    <div className="flex items-center gap-1 shrink-0 ml-1">
                      <Button
                        ...
                        size="icon"
                        variant="ghost"
                        className="h-7 w-7 rounded-md text-emerald-500 hover:text-emerald-600 hover:bg-emerald-500/10"
                        disabled={isPending}
                      >
                        {isPending ? (
                  ...
                  <span
                    onClick={() =>
                      startEditing("minAverageScore", values.minAverageScore)
                    }
            ...
            <div className="flex flex-col justify-between gap-3 rounded-xl border p-4 bg-card">
              <div className="flex items-center justify-between text-sm font-medium text-muted-foreground">
                ...
                <Tooltip>
                  ...
                  />
                  <TooltipContent>
                    <p>Rata-rata skor keseluruhan</p>
                  </TooltipContent>
                </Tooltip>
              </div>
              <div className="min-h-8 flex items-center">
